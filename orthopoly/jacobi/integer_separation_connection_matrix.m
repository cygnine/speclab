...
persistent input_schema one_minus_r_squared_times_p one_minus_r_times_p
persistent one_plus_r_times_p
if isempty(input_schema)
  from labtools import input_schema
  from speclab.orthopoly.jacobi import one_minus_r_squared_times_p ...
                                       one_minus_r_times_p ...
                                       one_plus_r_times_p
end
...
for q = 1:ab_min
  mu = one_minus_r_squared_times_p(ns,alpha+q,beta+q,opt);
...
if A>ab_min
  Q = A - ab_min;
  for q = 1:Q
    mu = one_minus_r_times_p(ns,alpha+q,beta,opt);
...
elseif B>ab_min
  Q = B - ab_min;
  for q = 1:Q
    mu = one_plus_r_times_p(ns,alpha,beta+q,opt);
    C_temp = spdiags(mu,[0,-1],N,N).';
    C = C_temp*C;
  end
end
