function[c] = least_opoly_lucoeffs(l,u,p,v,k,D,f, varargin)
% least_opoly_lucoeffs -- Computes basis coefficients for interpolation
%
% c = least_opoly_lucoeffs(l,u,p,v,k,dim,f, [ip=alpha!, connection=speye])
%
...
persistent invu invl dim subdim ip spdiag
...
if isempty(invu)
  ...
  [opt,parser] = input_parser({'ip', 'connection'}, {ip, @speye}, [], varargin{:});
else
...
for q = 0:k(end);
  % Determine the columns of G associated with this degree
