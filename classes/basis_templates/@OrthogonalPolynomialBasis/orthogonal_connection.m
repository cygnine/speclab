...
lmbda(1,1) = beta(1)/b(1);
for n = 2:N
  if n==2 % Ignore n=0 term in recurrence
    % For m=1, need to ignore m=0 term
    lmbda(2,1) = 1/b(2)*(alpha(1) - a(2))*lmbda(1,1);
    lmbda(2,2) = 1/b(2)*((alpha(1) - a(2))*lmbda(1,2) + beta(2)*lmbda(1,1));
  else
    % For m=1, need to ignore m=0 term
    lmbda(n,1) = 1/b(n)*( -b(n-1)*lmbda(n-2,1) + ...
                                (alpha(1) - a(n-1))*lmbda(n-1,1) + ...
                                beta(2)*lmbda(n-1,2) );

    % Can vectorize the rest
    lmbda(n,2:n) = 1/b(n)*(-b(n-1)*lmbda(n-2,2:n) + ...
                                  beta(2:n).*lmbda(n-1,1:n-1) + ...
                                  (alpha(2:n) - a(n-1)).*lmbda(n-1,2:n) + ...
                                  beta(3:n+1).*lmbda(n-1,3:n+1));
  end
end
...
k = self.norm(self.range(N));
kappa = other.norm(other.range(N));
