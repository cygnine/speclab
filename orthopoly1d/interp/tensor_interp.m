function[fz] = tensor_interp(theta, f, z, varargin)
% tensor_interp -- Computes multidimensional interpolation via de Boor's algorithm
%
% fz = tensor_interp(theta, f, z, [tol=1e-10, ip=alpha!, basis=multimonomial])
%
...
persistent tensor_lu tensor_coeffs multimonomial monomial_ip
persistent strict_inputs
if isempty(strict_inputs)
  from labtools import strict_inputs
  
  from speclab.orthopoly1d.interp import tensor_lu tensor_coeffs
...
[l,W,p,k,u] = tensor_lu(theta, opt);

c = tensor_coeffs(l,W,p,k,u,dim,f,'ip',opt.ip,'tol',opt.tol);
